...
function getPerspectiveLabel(perspective: Perspective | null) {
  return perspective ? PERSPECTIVE_LABELS[perspective] : '未定视角'
}
...
export function resolveStoryJourney(input: StoryJourneyInput): StoryJourneyResult {
  ...
  return {
    ...
    title: action.title,
    text: `${formatWorldTime(input.clock)}，你${action.verb}第 ${input.volume} 卷${perspectiveLabel}，${chapterText}已写入行程。${getMapLine(input.currentNodeMap)}，已完成 ${input.completedCount} 个节点。`,
    rewards: [
...
export function resolveStoryJourneyFeedback(input: StoryJourneyInput): StoryJourneyResult {
  return resolveStoryJourney(input)
}

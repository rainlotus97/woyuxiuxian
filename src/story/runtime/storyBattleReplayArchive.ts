...
function cloneEvent(event: BattleReplayEvent): BattleReplayEvent {
...
function resolveResultLabel(result: StoryBattleReplaySummary['result']): string {
...
function readArchive(): StoryBattleReplayRecord[] {
  ...
  try {
    ...
    memoryArchive = Array.isArray(parsed) ? parsed.map(cloneRecord) : []
...
function writeArchive(records: StoryBattleReplayRecord[]): void {
...
export function createStoryBattleReplayRecord(input: CreateStoryBattleReplayRecordInput): StoryBattleReplayRecord {
  ...
  const resultLabel = resolveResultLabel(input.result)
  ...
  return {
    ...
    subtitle: input.subtitle || `${resultLabel} · ${turns || 1} 手 · ${events.length} 条战况`,
...
export function saveStoryBattleReplayRecord(record: StoryBattleReplayRecord): void {

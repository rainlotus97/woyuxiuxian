  ...
  <div class="loop-hub-panel">
    <div class="loop-readiness-strip">
      ...
      <div class="readiness-counts">
        <span class="state-ready">可行动 {{ readiness.counts.ready }}</span>
        <span class="state-warning">需处理 {{ readiness.counts.warning }}</span>
        <span class="state-blocked">阻塞 {{ readiness.counts.blocked }}</span>
        <span class="state-closed">闭环 {{ closure.counts.closed }}/{{ closure.totalCount }}</span>
      </div>
    </div>

    <div class="p0-acceptance-panel" :class="`state-${acceptance.state}`">
      <div class="acceptance-copy">
        <span>{{ acceptance.gateLabel }}</span>
        <strong>{{ acceptance.headline }}</strong>
        <small v-if="acceptance.primaryGap">
          当前缺口：{{ acceptance.primaryGap.label }} · {{ acceptance.primaryGap.detail }}
        </small>
      ...
      <div class="acceptance-side">
        <div class="acceptance-gap-list" aria-label="P0 验收缺口">
          ...
          <span v-if="acceptance.readyForP1" class="gap-closed">P0 验收完成</span>
        </div>
        <button
